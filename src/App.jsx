...
export default function CCTVWithSensors() {
  ...
  const GO2RTC_URL = "http://114.9.13.244:1984"; // Dummy MQTT Data Generator with MORE realistic gradual changes
  ...
  useEffect(() => {
    ...
    let currentRainValue = 0;
    let currentWindKmh = 6.5;

    const generateRealisticData = () => {
      // Gradual temperature change: ±0.1 to ±0.2°C per update
      const tempChange = (Math.random() - 0.5) * 0.4; // <<< DIUBAH (sebelumnya 1.0)
      currentTemp = Math.max(20, Math.min(38, currentTemp + tempChange)); // Gradual humidity change: ±0.5 to ±1.0% per update
      const humidityChange = (Math.random() - 0.5) * 2; // <<< DIUBAH (sebelumnya 4)
      currentHumidity = Math.max(
        30,
        Math.min(95, currentHumidity + humidityChange)
      ); // Rain: mostly stays the same, occasional changes
      if (Math.random() < 0.1) {
        // 10% chance to change (sebelumnya 15%)
        const rainChange = (Math.random() - 0.5) * 10; // <<< DIUBAH (sebelumnya 20)
        currentRainValue = Math.max(
          0,
          Math.min(100, currentRainValue + rainChange)
        );
      } // Determine rain description
      let rainDesc = "";
      if (currentRainValue === 0) rainDesc = "No Rain";
      else if (currentRainValue <= 20) rainDesc = "Light Rain";
      else if (currentRainValue <= 50) rainDesc = "Moderate Rain";
      else if (currentRainValue <= 80) rainDesc = "Heavy Rain";
      else rainDesc = "Very Heavy Rain";
      const rain = `${Math.round(currentRainValue)} (${rainDesc})`; // Gradual wind change: ±0.5 to ±1.5 km/h per update
      const windChange = (Math.random() - 0.5) * 3; // <<< DIUBAH (sebelumnya 4)
      currentWindKmh = Math.max(5, Math.min(50, currentWindKmh + windChange));
      return {
        temperature: parseFloat(currentTemp.toFixed(1)),
        humidity: parseFloat(currentHumidity.toFixed(1)),
        rain,
        wind: parseFloat(currentWindKmh.toFixed(1)),
    ...
    setSensorData(generateRealisticData());
    console.log("✅ Using MORE realistic dummy MQTT data"); // Update dummy data every 3 seconds with gradual changes

    const interval = setInterval(() => {
      const newData = generateRealisticData();
      setSensorData(newData);
      console.log("📊 Dummy data updated:", newData);
    }, 3000);

    return () => clearInterval(interval);
  }, []);
  /* 
  // REAL MQTT Connection using WebSocket - COMMENTED OUT
  useEffect(() => {
    let client = null;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Real-time surveillance & weather monitoring
              </p>
            </div>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {getStatusText(status)} • {stream.location}
                      </p>
                    </div>
                  </div>
